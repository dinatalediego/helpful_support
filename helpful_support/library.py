...
def build_index(db_path: Path = DEFAULT_DB, root: Path = ROOT) -> int:
    ...
    # A sqlite3.Connection context manager commits or rolls back, but does not
    # close the handle. closing() is required so Windows can release the file.
    with closing(sqlite3.connect(db_path)) as con:
        with con:
            con.execute("DROP TABLE IF EXISTS library")
            con.execute(
                "CREATE VIRTUAL TABLE library USING "
                "fts5(source, title, section, content, tags, tokenize='unicode61')"
            )
            con.executemany(
